...
def gbt_to_new_template(gbt: dict, extranonce_len: int = 8) -> dict:
    ...
    _h = bip34_height_push(gbt["height"])   # returns len_byte || script_bytes
    prefix = _h[1:]                          # SV2 B0255 adds its own length
...
def reconstruct_block_from_coinbase(tmpl: dict, coinbase_tx: bytes,
    ...
    pre = bytes([len(tmpl["coinbase_prefix"])]) + tmpl["coinbase_prefix"]
